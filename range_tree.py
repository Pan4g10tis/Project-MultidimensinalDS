        ...
        for row in readCSV:
            ...
            if len(indices) == 1:
                ...
                if indices[0] == 7:
                    ...
                    x1 = date_to_numeric(row[indices[0]])
                    value = float(x1) if row[indices[0]] != '' else 0.0
                ...
                for i in indices:
                    ...
                    if header[i].lower() == 'review_date':  
                        x1 = date_to_numeric(row[i])
                        value = float(x1) if row[i] != '' else 0.0
                        values.append(value)
...
def range_tree_main():
    ...
    print("Choose some of these categories for indexing", headings)
    Choices = input("Headings : ")
    input_list = Choices.split()
    categories, extras, dim = category_reader(input_list, headings)
    search = []

    range, extra_range = range_reader(categories, extras)
    ...
    if dim == 0:
        ...
        search += SearchRangeTree1d(tree, range[0][0], range[0][1], dim)
    elif dim == 2:
        tree = ConstructRangeTree2d(data)
        search += SearchRangeTree2d(tree, range[0][0], range[0][1], range[1][0], range[1][1], dim)
    elif dim == 3:
        tree = ConstructRangeTree3d(data)
        search += SearchRangeTree3d(tree, range[0][0], range[0][1], range[1][0], range[1][1], range[2][0], range[2][1], dim)

    end = time.time()
    length = end - start

    if len(extra_range) != 0:
        search = other_categories(search, headings, extra_range, extras)

    print(length)

    user_answer = input("Do you want the results to be sorted by a certain category? [Y/N]")
    ...
    if user_answer == 'N':
        ...
        print("Which of these categories do you want chose:", headings)
        user_input = input()
        while user_input not in headings:
            print("This category doesn't exist in the file: ", user_input)
            user_input = input("Please enter another category:")
        col_index = headings.index(user_input)
        if col_index == 7:
            search = sorted(search, key=lambda x: float(date_to_numeric(x[7])))
        else:
            search = sorted(search, key=lambda x: float(x[col_index]))
