...
def rtree_main(selected_attributes, conditions, review_keywords=None, num_neighbors=None):
    ...
    :param review_keywords: List of keywords to search for in reviews (optional).
    :param num_neighbors: Number of nearest neighbors to return (optional).
    ...
    if not selected_numeric:
        print("No numeric attributes selected for R-tree. Search will be performed linearly based on non-numeric attributes.")
    else:
        # Create the R-tree
    ...
    if not parsed_conditions and not non_numeric_conditions and not review_keywords:
    ...
    if matching_entries:
        ...
        # If review_keywords are provided, perform LSH-based search on the filtered results
        if review_keywords:
            ...
            # Extract the rows from the LSH results
