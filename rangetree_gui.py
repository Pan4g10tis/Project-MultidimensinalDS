...
from range_tree import range_tree_main
...
class RangeTreeGUI:
    def __init__(self, root):
        ...
        self.headings = ['name', 'roaster', 'roast', 'loc_country', 'origin', '100g_USD', 'rating', 'review_date', 'review']
    ...
    def create_widgets(self):
        ...
        self.results_tree.heading("Loc Country", text="Loc Country",
                                  command=lambda: self.sort_treeview("Loc Country", False))
    ...
    def perform_search(self):
        ...
        # Call the range tree main function with selected attributes and conditions
        results = range_tree_main(self.selected_attributes, self.conditions)
    ...
    def display_results(self, results):
        ...
        if results:
            for row in results:
                ...
                review_date_index = self.headings.index("review_date")
                review_date_str = row[review_date_index]
                try:
                    review_date_numeric = int(datetime.strptime(review_date_str, "%B %Y").strftime("%Y%m"))
                except ValueError:
                    review_date_numeric = review_date_str  # Fallback to original value if conversion fails

                # Replace the 'review_date' value in the row with the numeric format
                row = list(row)  # Convert tuple to list for modification
                row[review_date_index] = review_date_numeric
